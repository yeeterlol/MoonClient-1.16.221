...
namespace Gma.System.MouseKeyHook.Implementation
{
	internal abstract class EventFacade : IKeyboardMouseEvents, IKeyboardEvents, IMouseEvents, IDisposable
	{
		...
		public event EventHandler<MouseEventExtArgs> MouseWheelExt
		{
			add
			{
				GetMouseListener().MouseWheelExt += value;
			}
			remove
			{
				GetMouseListener().MouseWheelExt -= value;
		...
		public event MouseEventHandler MouseDragStarted
		{
			add
			{
				GetMouseListener().MouseDragStarted += value;
			}
			remove
			{
				GetMouseListener().MouseDragStarted -= value;
		...
		public event EventHandler<MouseEventExtArgs> MouseDragStartedExt
		{
			add
			{
				GetMouseListener().MouseDragStartedExt += value;
			}
			remove
			{
				GetMouseListener().MouseDragStartedExt -= value;
		...
		public event MouseEventHandler MouseDragFinished
		{
			add
			{
				GetMouseListener().MouseDragFinished += value;
			}
			remove
			{
				GetMouseListener().MouseDragFinished -= value;
		...
		public event EventHandler<MouseEventExtArgs> MouseDragFinishedExt
		{
			add
			{
				GetMouseListener().MouseDragFinishedExt += value;
			}
			remove
			{
				GetMouseListener().MouseDragFinishedExt -= value;
